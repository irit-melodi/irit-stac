...
if [ $# -gt 2 ]; then
    echo >&2 "Usage: $0 file.soclog.seg.csv gen"
    exit 1
fi
...
python "$CODE_DIR/intake/csvtoglozz.py" --gen "$GEN" -f sanity-check/unsegmented.csv
python "$CODE_DIR/intake/csvtoglozz.py" --gen "$GEN" -f sanity-check/segmented.csv
...
for i in sections/*.soclog.seg.csv; do
    echo $i
    python "$CODE_DIR/intake/csvtoglozz.py" --gen "$GEN" -f "$i"
done
